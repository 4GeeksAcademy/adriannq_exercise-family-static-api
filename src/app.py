...
@app.route('/member/<int:member_id>', methods=['DELETE',"GET"])
def handle_delete_member(member_id):
    member= jackson_family.get_member(member_id)
    deleted_member = jackson_family.delete_member(member_id)
    if deleted_member == "ok":
        return jsonify(f"{member['first_name']} deleted successfully"), 200
    return jsonify(deleted_member), 400
